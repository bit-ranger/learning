...
(define lookup
  (lambda (x env)
    (let ([p (assq x env)])                            ;找出环境链中第一次出现的以x对应符号为car的序对。
      (cond
       [(not p) x]
       [else (cdr p)]))))
...
(define interp1
  (lambda (exp env)
    (match exp                                          ; 模式匹配 exp 的以下情况（分支）
      ...
      [(? number? x) x]                                 ; 数字
      [`(lambda (,x) ,e)                                ; 函数，闭包表达式与环境
       (Closure exp env)]
      [`(,e1 ,e2)                                       ; 调用，分别求值
       (let ([v1 (interp1 e1 env)]                      ; 从当前环境中取出变量值
             [v2 (interp1 e2 env)])
         (match v1
           [(Closure `(lambda (,x) ,e) env1)            ; 拆解闭包，得到表达式与环境
            (interp1 e (ext-env x v2 env1))]))]         ; 扩展闭包的环境, 将形参与实参绑定, 对表达式进行解释
                                                        ; 函数调用完成后，得到的结果将绑定到当前环境，扩展的闭包环境自生自灭。
